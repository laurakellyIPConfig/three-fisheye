...
export abstract class Fisheye<Camera extends THREE.Camera> {
  ...
  constructor(
    camera: Camera,
    o?: { textureSizeExponent?: number; mesh?: number; debug?: boolean }
  ) {
    ...
    this.exponent = 0;
    this.texctx = <CanvasRenderingContext2D>(
  ...
  public resize(): void {
    ...
    console.log("exponent choice", this.exponent);
    this.updateFisheyeRegion();
  }
...
export function load_skybox_texture(
...
export function createSkyboxMesh(
  skybox_texture: THREE.CubeTexture
): THREE.Mesh {
 ...
 * @return [x, y]
 */
export function sphere2Mercator(
  longitude: Radian,
  latitude: Radian
): [number, number] {
  const x = longitude;
 ...
 */
export function fisheye2Sphere(
  x: number,
  y: number,
  r = 1
): [Radian, Radian] | null {
 ...
 * @return [x, y] ∈ [-1, 1]
 */
export function sphere2Fisheye(
  longitude: Radian,
  latitude: Radian,
  r = 1
): [number, number] {
 ...
 */
export function fisheye2equirectangular(
  x: number,
  y: number
): [number, number] {
  const [w, h] = [1, 1];
